...
# --- 2. 日志函数 --- (更新为CIFAR100)
log_filename = os.path.join(log_dir, f"qat_resnet18_c100_optimized_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
log_message(f"Environment: {device} | Task: CIFAR-100 QAT | Batch Size: {batch_size} | Epochs: {epochs} | Engine: qnnpack")

# --- 3. 数据处理 --- (保持与用户原有CIFAR100设置一致)
data_dir = '/root/autodl-tmp/data' if os.path.exists("/root/autodl-tmp/data") else './data'
...
def progressive_qat_training(model, train_loader, test_loader, epochs, optimizer, criterion, device):
    ...
    """
    best_acc = 0.0
    best_qat_path = os.path.join(model_dir, "resnet18_c100_qat_progressive_best.pth")
    ...
    for epoch in range(epochs):
        ...
        if epoch < 3:
            ...
            # 解冻BN统计信息的正确方法（修复PyTorch版本兼容性问题）
...
# --- 7. 模型加载与 QAT 准备 --- (保持与用户原有CIFAR100设置一致)
model = models.quantization.resnet18(weights=None, quantize=False)
model.fc = nn.Linear(model.fc.in_features, 100)  # CIFAR100需要100个类别
...
# --- 12. 最终转换与部署导出 --- (保持与用户原有CIFAR100设置一致)
...
# 定义路径（保持与用户原有CIFAR100命名一致）
weights_path = os.path.join(model_dir, "resnet18_c100_int8_final_optimized.pth")
deploy_path = os.path.join(model_dir, "resnet18_c100_int8_deploy_optimized.pt")
...
log_message("=" * 70)
log_message(f"ResNet18 CIFAR-100 Optimized QAT Results")
log_message("=" * 70)
