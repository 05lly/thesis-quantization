...
# --- 2. 日志函数 --- 
log_filename = os.path.join(log_dir, f"vgg16_c100_int4_qat_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
model.classifier[6] = nn.Linear(4096, 100)  # CIFAR-100有100个类别

# 加载FP32预训练模型
fp32_path = os.path.join(model_dir, "vgg16_c100_fp32_best.pth")
...
# 定义输出路径
deploy_path = os.path.join(model_dir, "vgg16_c100_int4_qat_deploy.pt")
...
log_message("=" * 70)
log_message("VGG16 CIFAR-100 INT4 QAT Results")
log_message("=" * 70)
log_message(f"FP32 Accuracy: {fp32_acc:.2f}%")
