...
log_filename = os.path.join(
    log_dir,
    f"qat_int4_mnv2_c10_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
...
# --- 3. 模型准备 ---
log_message("Initializing MobileNetV2 for INT4 QAT (CIFAR-10)...")
...
for epoch in range(epochs):
    ...
    if val_acc > best_acc:
        best_acc = val_acc
        torch.save(model.state_dict(), os.path.join(model_dir, "mobilenetv2_int4_c10_best.pth"))

# --- 6. 总结 ---
fp32_size = os.path.getsize(fp32_path) / (1024 * 1024)

log_message("=" * 60)
log_message(" MobileNetV2 INT4 QAT Final Report (CIFAR-10) ")
