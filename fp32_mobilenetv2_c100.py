...
# --- 4. 数据处理 ---
transform_std = transforms.Normalize((0.5071, 0.4865, 0.4409), (0.2673, 0.2564, 0.2761))
trainloader = torch.utils.data.DataLoader(
    datasets.CIFAR100('./data', train=True, download=True, transform=transforms.Compose([transforms.Resize(224), transforms.RandomHorizontalFlip(), transforms.ToTensor(), transform_std])),
    batch_size=batch_size, shuffle=True, num_workers=4)

testloader = torch.utils.data.DataLoader(
    datasets.CIFAR100('./data', train=False, download=True, transform=transforms.Compose([transforms.Resize(224), transforms.ToTensor(), transform_std])),
...
for epoch in range(epochs):
    model.train()
    correct, total = 0, 0
    for inputs, labels in tqdm(trainloader, desc=f"FP32 MBV2 Epoch {epoch+1}", leave=False):
    ...
    with torch.no_grad():
        for inputs, labels in testloader:
            ...
            outputs = model(inputs)
            _, pred = torch.max(outputs, 1)
            t_tot += labels.size(0); t_corr += (pred == labels).sum().item()
