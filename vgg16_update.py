...
# --- 3. 模型准备  ---
log_message("Initializing VGG16 for Mixed Precision (INT4+FP32) QAT (CIFAR-100)...")
...
# 2. 保护首尾层：将它们的 qconfig 设为 None，强制跳过量化，保持 FP32 精度
model.features[0].qconfig = None   # 保护第一层卷积 (3->64)
model.classifier[6].qconfig = None # 保护最后一层全连接 (4096->100)
...
log_message("=" * 60)
log_message(f" VGG16 Mixed Precision INT4 QAT Final Report ")
log_message(f" Best Val Accuracy : {best_acc:.2f}%")
log_message(f" Theory INT4 Size  : {fp32_size / 8:.2f} MB (Approximation)")
