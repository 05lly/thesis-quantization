...
# --- 2. 日志函数 --- 
log_filename = os.path.join(log_dir, f"resnet18_c100_int4_qat_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
model.fc = nn.Linear(model.fc.in_features, 100)  # CIFAR-100有100个类别
...
# 定义输出路径
deploy_path = os.path.join(model_dir, "resnet18_c100_int4_qat_deploy.pt")
...
log_message("=" * 70)
log_message("ResNet18 CIFAR-100 INT4 QAT Results")
log_message("=" * 70)
log_message(f"FP32 Accuracy: {fp32_acc:.2f}%")
