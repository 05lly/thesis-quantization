...
trainloader = torch.utils.data.DataLoader(datasets.CIFAR10('./data', train=True, download=True, transform=transform), batch_size=batch_size, shuffle=True, num_workers=4)
testloader = torch.utils.data.DataLoader(datasets.CIFAR10('./data', train=False, download=True, transform=transform), batch_size=batch_size, shuffle=False, num_workers=4)

# --- 5. 模型初始化与 QAT ---
log_message("Initializing VGG16 INT4 QAT...")
...
for epoch in range(epochs):
    ...
    correct, total = 0, 0
    for inputs, labels in tqdm(trainloader, desc=f"INT4 QAT VGG16 Epoch {epoch+1}"):
    ...
    log_message(f"Epoch {epoch+1} | TestAcc: {val_acc:.2f}%")
    if val_acc > best_acc:
        best_acc = val_acc
...
log_message("=" * 55)
log_message(f"VGG16 INT4 QAT Report")
