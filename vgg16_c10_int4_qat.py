...
batch_size = 64  # VGG16模型较大，使用较小的batch_size
...
# --- 2. 日志函数 --- 
log_filename = os.path.join(log_dir, f"vgg16_c10_int4_qat_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
log_message(f"Environment: {device} | Dataset: CIFAR-10 | Batch Size: {batch_size} | Epochs: {epochs} | Engine: qnnpack")
log_message("INT4 Quantization Aware Training (QAT) - VGG16")
...
# --- 5. 加载FP32模型 --- 
log_message("Loading FP32 VGG16 model...")

# 创建VGG16模型
model = models.vgg16(weights=None)
model.classifier[6] = nn.Linear(4096, 10)  # CIFAR-10有10个类别

# 加载FP32预训练模型
fp32_path = os.path.join(model_dir, "vgg16_c10_fp32_best.pth")
...
# 定义输出路径
deploy_path = os.path.join(model_dir, "vgg16_c10_int4_qat_deploy.pt")
...
log_message("=" * 70)
log_message("VGG16 CIFAR-10 INT4 QAT Results")
log_message("=" * 70)
log_message(f"FP32 Accuracy: {fp32_acc:.2f}%")
