...
log_filename = os.path.join(log_dir, f"qat_int4_resnet18_c100_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
def log_message(msg):
...
for epoch in range(epochs):
    ...
    if val_acc > best_acc:
        best_acc = val_acc
        torch.save(model.state_dict(), os.path.join(model_dir, "resnet18_int4_c100_best.pth"))

fp32_size = os.path.getsize(fp32_path) / (1024 * 1024)
log_message("=" * 60)
log_message(f" ResNet18 INT4 QAT Final Report ")
